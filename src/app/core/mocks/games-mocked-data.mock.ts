import { GameModel } from '@core/models/game.model';

export const gamesMockedData: GameModel[] = [
  {
    id: 0,
    name: 'Doom',
    price: 666,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      'Doom is a 1993 first-person shooter (FPS) game developed by id Software for MS-DOS. Players assume the role of a space marine, popularly known as Doomguy, fighting their way through hordes of invading demons from Hell. The first episode, comprising nine levels, was distributed freely as shareware and played by an estimated 15–20 million people within two years; the full game, with two further episodes, was sold via mail order. An updated version with an additional episode and more difficult levels, The Ultimate Doom, was released in 1995 and sold at retail.',
  },
  {
    id: 1,
    name: 'Duke Nukem 3D',
    price: 249,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      'This classic game is a first person shooter (fps) that was created and published by the company 3D Realms in 1996. There are 28 levels spread through 3 episodes in the original version of this game.  A feature that was very unique for a game in 1996, is that the player is actually placed in the role of Duke, the hypermasculine action hero. The sequel of the game series see Duke back on Earth fighting aliens that have taken over the planet and gotten rid of the men leaving the world in shambles.',
  },
  {
    id: 2,
    name: 'Wildermyth',
    price: 299,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Wildermyth is a character-driven, procedurally-generated tactical RPG. Like the best tabletop roleplaying experiences, Wildermyth gives you choices and answers your every decision with consequences that drive your characters forward. Lead a band of heroes as they grow from reluctant farmers into unique, legendary fighters. Combat unexpected threats and strange monsters across interactive battlefields. Unravel mysteries and share pensive moments in an ever-new fantasy setting that blends hard truths and sacrifice with humor and personal storytelling.',
  },
  {
    id: 3,
    name: '20000 Leagues Under the Sea',
    price: 399,
    currencyCode: 'UAH',
    tag: 'Adventure',
    description:
      'Based on the Jules Verne novel, this graphic adventure provides a first-person perspective on the realm of Captain Nemo and his enormous submarine craft, The Nautilus. Players guide three brave explorers -- marine biologist Pierre Aronnax, his assistant Conseil, and the rough-and-tumble seaman Ned Land -- who had boarded the warship Abraham Lincoln and embarked on a mission to hunt down an impossibly enormous "sea monster" reported by several merchant ships.',
  },
  {
    id: 4,
    name: 'Skulls of the Toltecs',
    price: 449,
    currencyCode: 'UAH',
    tag: 'Adventure',
    description:
      "Released in 1996, 3 Skulls of the Toltecs is a cartoon adventure game in the style of LucasArts' The Secret of Monkey Island. Unlike the popular Monkey Island, however, this game seems to have had very little impact on the adventure playing community. Perhaps its style was already considered dated by the time of its release, or maybe its 'wild west' scenario just didn't strike the right chord with many adventurers. I must admit I was put off by this latter aspect when I passed up the opportunity to play it about twelve months ago. Fortunately for me, a friend insisted that I at least give it a try and I'm glad that I did. As new adventure games are scarce at the moment I was in no position to turn up my nose a second time.",
  },
  {
    id: 5,
    name: 'AGON',
    price: 249,
    currencyCode: 'UAH',
    tag: 'Adventure',
    description:
      "Agon: The Mysterious Codex is a collection of three episodes in a planned 14-chapter adventure game by Hungarian developer Private Moon Studios. Players are cast in the role of Professor Samuel Hunt, who stumbles upon a mysterious artifact and some torn codex pages while working for the British Museum in 1903. This kicks off a global adventure that begins in London and will take him to Lapland and Madagascar to discover the artifact's origin and purpose. The game offers 360° panoramic views of pre-rendered environments with a traditional mouse-driven interface to move, view, and interact with objects. Each episode spans an estimated two-to-three hours in length, with more chapters available from the developer's website.",
  },
  {
    id: 6,
    name: 'Heretic II',
    price: 199,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      'Heretic II is a dark fantasy action-adventure game developed by Raven Software and published by Activision in 1998 continuing the story of Corvus, the main character from its predecessor, Heretic. It is the fourth game in the Hexen: Beyond Heretic series and comes after the "Serpent Rider" trilogy. Although Id Software owns the publishing rights to the previous titles, Heretic 2 is owned by Activision since they own Raven Software and its IPs.',
  },
  {
    id: 7,
    name: 'The Terminator: Rampage',
    price: 449,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      "Skynet has sent a computer core containing its core programming back to 1984, shortly before its ultimate defeat at the hands of John Connor's human resistance in 2024. The computer core arrives at Cyberdyne Systems' headquarters at the Cheyenne Mountain Complex, and proceeds to take over the building and begin manufacturing an army of Terminators. A lone commando is sent into the past by John Connor, arriving there in 1988. His mission is to destroy the Skynet computer core and eliminate the threat of Skynet once and for all. To do so, players must explore the 32 floors of the Cyberdyne building, fighting off various Skynet robots and cyborgs while assembling the pieces of a prototype plasma weapon called the V-TEC PPC (Phased Plasma Cannon), the only means of destroying the Meta-Node."
  },
  {
    id: 8,
    name: 'Bow Street Runner',
    price: 499,
    currencyCode: 'UAH',
    tag: 'Adventure',
    description:
      "Bow Street Runner is a mystery adventure series created by LittleLoud and commissioned by Channel 4 (UK) to promote the five-part City of Vice programs. Set in London’s Covent Garden in the 1750s, crime in the city has risen to such a level that local magistrates were forced to take action. The Bow Street Runners were born, created to instill order in the increasingly crime-ridden city. It’s a dark and historically accurate journey through old London, presented with live actors, voice narration and full motion video for an impressive cinematic experience."
  },
  {
    id: 9,
    name: 'Redneck Rampage',
    price: 599,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      "Well, we crash-landed that UFO in the desert and Bubba says he wants to go home. Heck, I don't blame him 'cause there ain't no place like Hickston. But according to my map, we're gonna have to blast our way through jack o' lope farms and a riverboat as well as a brothel 'fore we get home again."
  },
  {
    id: 10,
    name: 'Star Wars: Dark Forces',
    price: 399,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      "Star Wars: Dark Forces is a first-person shooter video game developed and published by LucasArts. It was released in 1995 for MS-DOS and Macintosh, and in 1996 for the PlayStation. The story is set in the fictional Star Wars expanded universe and begins shortly before the original Star Wars film, before flashing forward to a year after the film's events. The game's protagonist and playable character is Kyle Katarn, a mercenary working on behalf of the Rebel Alliance who discovers the Galactic Empire's secret Dark Trooper Project, which involves the development of a series of powerful new battle droids and power-armored stormtroopers."
  },
  {
    id: 11,
    name: 'Star Wars: Battlefront',
    price: 999,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      'STAR WARS™ Battlefront is an action/shooter game that gives fans and gamers the opportunity to re-live and participate in all of the classic Star Wars battles like never before. Players can select one of a number of different soldier types, jump into any vehicle, man any turret on the battlefront and conquer the galaxy planet-by-planet online with their friends or offline in a variety of single player modes. Single player modes include "Instant Action", "Galactic Conquest" and the story-based "Historical Campaigns" mode that lets gamers experience all of the epic Star Wars battles from Episodes I-VI, fighting from the perspective of each of the four factions within the game.'
  },
  {
    id: 12,
    name: 'Doom 2',
    price: 666,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      'Doom is a 1993 first-person shooter (FPS) game developed by id Software for MS-DOS. Players assume the role of a space marine, popularly known as Doomguy, fighting their way through hordes of invading demons from Hell. The first episode, comprising nine levels, was distributed freely as shareware and played by an estimated 15–20 million people within two years; the full game, with two further episodes, was sold via mail order. An updated version with an additional episode and more difficult levels, The Ultimate Doom, was released in 1995 and sold at retail.',
  },
  {
    id: 13,
    name: 'Duke Nukem 2',
    price: 399,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      'This classic game is a first person shooter (fps) that was created and published by the company 3D Realms in 1996. There are 28 levels spread through 3 episodes in the original version of this game.  A feature that was very unique for a game in 1996, is that the player is actually placed in the role of Duke, the hypermasculine action hero. The sequel of the game series see Duke back on Earth fighting aliens that have taken over the planet and gotten rid of the men leaving the world in shambles.',
  },
  {
    id: 14,
    name: 'Wildermyth 2',
    price: 349,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Wildermyth is a character-driven, procedurally-generated tactical RPG. Like the best tabletop roleplaying experiences, Wildermyth gives you choices and answers your every decision with consequences that drive your characters forward. Lead a band of heroes as they grow from reluctant farmers into unique, legendary fighters. Combat unexpected threats and strange monsters across interactive battlefields. Unravel mysteries and share pensive moments in an ever-new fantasy setting that blends hard truths and sacrifice with humor and personal storytelling.',
  },
  {
    id: 15,
    name: '20000 Leagues Under the Sea 2',
    price: 399,
    currencyCode: 'UAH',
    tag: 'Adventure',
    description:
      'Based on the Jules Verne novel, this graphic adventure provides a first-person perspective on the realm of Captain Nemo and his enormous submarine craft, The Nautilus. Players guide three brave explorers -- marine biologist Pierre Aronnax, his assistant Conseil, and the rough-and-tumble seaman Ned Land -- who had boarded the warship Abraham Lincoln and embarked on a mission to hunt down an impossibly enormous "sea monster" reported by several merchant ships.',
  },
  {
    id: 16,
    name: 'Skulls of the Toltecs 2',
    price: 749,
    currencyCode: 'UAH',
    tag: 'Adventure',
    description:
      "Released in 1996, 3 Skulls of the Toltecs is a cartoon adventure game in the style of LucasArts' The Secret of Monkey Island. Unlike the popular Monkey Island, however, this game seems to have had very little impact on the adventure playing community. Perhaps its style was already considered dated by the time of its release, or maybe its 'wild west' scenario just didn't strike the right chord with many adventurers. I must admit I was put off by this latter aspect when I passed up the opportunity to play it about twelve months ago. Fortunately for me, a friend insisted that I at least give it a try and I'm glad that I did. As new adventure games are scarce at the moment I was in no position to turn up my nose a second time.",
  },
  {
    id: 17,
    name: 'AGON 2',
    price: 849,
    currencyCode: 'UAH',
    tag: 'Adventure',
    description:
      "Agon: The Mysterious Codex is a collection of three episodes in a planned 14-chapter adventure game by Hungarian developer Private Moon Studios. Players are cast in the role of Professor Samuel Hunt, who stumbles upon a mysterious artifact and some torn codex pages while working for the British Museum in 1903. This kicks off a global adventure that begins in London and will take him to Lapland and Madagascar to discover the artifact's origin and purpose. The game offers 360° panoramic views of pre-rendered environments with a traditional mouse-driven interface to move, view, and interact with objects. Each episode spans an estimated two-to-three hours in length, with more chapters available from the developer's website.",
  },
  {
    id: 18,
    name: 'Heretic III',
    price: 299,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      'Heretic II is a dark fantasy action-adventure game developed by Raven Software and published by Activision in 1998 continuing the story of Corvus, the main character from its predecessor, Heretic. It is the fourth game in the Hexen: Beyond Heretic series and comes after the "Serpent Rider" trilogy. Although Id Software owns the publishing rights to the previous titles, Heretic 2 is owned by Activision since they own Raven Software and its IPs.',
  },
  {
    id: 19,
    name: 'The Terminator: Rampage 2',
    price: 449,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      "Skynet has sent a computer core containing its core programming back to 1984, shortly before its ultimate defeat at the hands of John Connor's human resistance in 2024. The computer core arrives at Cyberdyne Systems' headquarters at the Cheyenne Mountain Complex, and proceeds to take over the building and begin manufacturing an army of Terminators. A lone commando is sent into the past by John Connor, arriving there in 1988. His mission is to destroy the Skynet computer core and eliminate the threat of Skynet once and for all. To do so, players must explore the 32 floors of the Cyberdyne building, fighting off various Skynet robots and cyborgs while assembling the pieces of a prototype plasma weapon called the V-TEC PPC (Phased Plasma Cannon), the only means of destroying the Meta-Node."
  },
  {
    id: 20,
    name: 'Bow Street Runner 2',
    price: 499,
    currencyCode: 'UAH',
    tag: 'Adventure',
    description:
      "Bow Street Runner is a mystery adventure series created by LittleLoud and commissioned by Channel 4 (UK) to promote the five-part City of Vice programs. Set in London’s Covent Garden in the 1750s, crime in the city has risen to such a level that local magistrates were forced to take action. The Bow Street Runners were born, created to instill order in the increasingly crime-ridden city. It’s a dark and historically accurate journey through old London, presented with live actors, voice narration and full motion video for an impressive cinematic experience."
  },
  {
    id: 21,
    name: 'Redneck Rampage 2',
    price: 699,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      "Well, we crash-landed that UFO in the desert and Bubba says he wants to go home. Heck, I don't blame him 'cause there ain't no place like Hickston. But according to my map, we're gonna have to blast our way through jack o' lope farms and a riverboat as well as a brothel 'fore we get home again."
  },
  {
    id: 22,
    name: 'Star Wars: Dark Forces 2',
    price: 799,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      "Star Wars: Dark Forces is a first-person shooter video game developed and published by LucasArts. It was released in 1995 for MS-DOS and Macintosh, and in 1996 for the PlayStation. The story is set in the fictional Star Wars expanded universe and begins shortly before the original Star Wars film, before flashing forward to a year after the film's events. The game's protagonist and playable character is Kyle Katarn, a mercenary working on behalf of the Rebel Alliance who discovers the Galactic Empire's secret Dark Trooper Project, which involves the development of a series of powerful new battle droids and power-armored stormtroopers."
  },
  {
    id: 23,
    name: 'Star Wars: Battlefront 2',
    price: 1599,
    currencyCode: 'UAH',
    tag: 'Action',
    description:
      'STAR WARS™ Battlefront is an action/shooter game that gives fans and gamers the opportunity to re-live and participate in all of the classic Star Wars battles like never before. Players can select one of a number of different soldier types, jump into any vehicle, man any turret on the battlefront and conquer the galaxy planet-by-planet online with their friends or offline in a variety of single player modes. Single player modes include "Instant Action", "Galactic Conquest" and the story-based "Historical Campaigns" mode that lets gamers experience all of the epic Star Wars battles from Episodes I-VI, fighting from the perspective of each of the four factions within the game.'
  },
  {
    id: 24,
    name: 'Game Title 1',
    price: 1399,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed viverra sollicitudin pellentesque. Maecenas suscipit, purus dictum congue condimentum, purus ante mollis leo, non tristique orci urna pretium nisl. Vestibulum nec nisi diam. Etiam viverra facilisis est non dignissim. Maecenas sodales elit quam, vel interdum mi placerat non. Quisque at metus faucibus mauris lobortis ultricies a sit amet quam. Mauris pretium neque sit amet sem porta, ornare dapibus mauris varius. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer facilisis nunc et pretium iaculis. Integer commodo risus id justo accumsan commodo. Sed sapien lacus, faucibus a elit a, commodo aliquet lectus.'
  },
  {
    id: 25,
    name: 'Game Title 2',
    price: 499,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed viverra sollicitudin pellentesque. Maecenas suscipit, purus dictum congue condimentum, purus ante mollis leo, non tristique orci urna pretium nisl. Vestibulum nec nisi diam. Etiam viverra facilisis est non dignissim. Maecenas sodales elit quam, vel interdum mi placerat non. Quisque at metus faucibus mauris lobortis ultricies a sit amet quam. Mauris pretium neque sit amet sem porta, ornare dapibus mauris varius. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer facilisis nunc et pretium iaculis. Integer commodo risus id justo accumsan commodo. Sed sapien lacus, faucibus a elit a, commodo aliquet lectus.'
  },
  {
    id: 26,
    name: 'Game Title 3',
    price: 799,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed viverra sollicitudin pellentesque. Maecenas suscipit, purus dictum congue condimentum, purus ante mollis leo, non tristique orci urna pretium nisl. Vestibulum nec nisi diam. Etiam viverra facilisis est non dignissim. Maecenas sodales elit quam, vel interdum mi placerat non. Quisque at metus faucibus mauris lobortis ultricies a sit amet quam. Mauris pretium neque sit amet sem porta, ornare dapibus mauris varius. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer facilisis nunc et pretium iaculis. Integer commodo risus id justo accumsan commodo. Sed sapien lacus, faucibus a elit a, commodo aliquet lectus.'
  },
  {
    id: 27,
    name: 'Game Title 4',
    price: 999,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed viverra sollicitudin pellentesque. Maecenas suscipit, purus dictum congue condimentum, purus ante mollis leo, non tristique orci urna pretium nisl. Vestibulum nec nisi diam. Etiam viverra facilisis est non dignissim. Maecenas sodales elit quam, vel interdum mi placerat non. Quisque at metus faucibus mauris lobortis ultricies a sit amet quam. Mauris pretium neque sit amet sem porta, ornare dapibus mauris varius. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer facilisis nunc et pretium iaculis. Integer commodo risus id justo accumsan commodo. Sed sapien lacus, faucibus a elit a, commodo aliquet lectus.'
  },
  {
    id: 28,
    name: 'Game Title 5',
    price: 899,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed viverra sollicitudin pellentesque. Maecenas suscipit, purus dictum congue condimentum, purus ante mollis leo, non tristique orci urna pretium nisl. Vestibulum nec nisi diam. Etiam viverra facilisis est non dignissim. Maecenas sodales elit quam, vel interdum mi placerat non. Quisque at metus faucibus mauris lobortis ultricies a sit amet quam. Mauris pretium neque sit amet sem porta, ornare dapibus mauris varius. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer facilisis nunc et pretium iaculis. Integer commodo risus id justo accumsan commodo. Sed sapien lacus, faucibus a elit a, commodo aliquet lectus.'
  },
  {
    id: 29,
    name: 'Game Title 6',
    price: 1799,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed viverra sollicitudin pellentesque. Maecenas suscipit, purus dictum congue condimentum, purus ante mollis leo, non tristique orci urna pretium nisl. Vestibulum nec nisi diam. Etiam viverra facilisis est non dignissim. Maecenas sodales elit quam, vel interdum mi placerat non. Quisque at metus faucibus mauris lobortis ultricies a sit amet quam. Mauris pretium neque sit amet sem porta, ornare dapibus mauris varius. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer facilisis nunc et pretium iaculis. Integer commodo risus id justo accumsan commodo. Sed sapien lacus, faucibus a elit a, commodo aliquet lectus.'
  },
  {
    id: 30,
    name: 'Game Title 7',
    price: 1499,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed viverra sollicitudin pellentesque. Maecenas suscipit, purus dictum congue condimentum, purus ante mollis leo, non tristique orci urna pretium nisl. Vestibulum nec nisi diam. Etiam viverra facilisis est non dignissim. Maecenas sodales elit quam, vel interdum mi placerat non. Quisque at metus faucibus mauris lobortis ultricies a sit amet quam. Mauris pretium neque sit amet sem porta, ornare dapibus mauris varius. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer facilisis nunc et pretium iaculis. Integer commodo risus id justo accumsan commodo. Sed sapien lacus, faucibus a elit a, commodo aliquet lectus.'
  },
  {
    id: 31,
    name: 'Game Title 8',
    price: 1899,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed viverra sollicitudin pellentesque. Maecenas suscipit, purus dictum congue condimentum, purus ante mollis leo, non tristique orci urna pretium nisl. Vestibulum nec nisi diam. Etiam viverra facilisis est non dignissim. Maecenas sodales elit quam, vel interdum mi placerat non. Quisque at metus faucibus mauris lobortis ultricies a sit amet quam. Mauris pretium neque sit amet sem porta, ornare dapibus mauris varius. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer facilisis nunc et pretium iaculis. Integer commodo risus id justo accumsan commodo. Sed sapien lacus, faucibus a elit a, commodo aliquet lectus.'
  },
  {
    id: 32,
    name: 'Game Title 9',
    price: 1999,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed viverra sollicitudin pellentesque. Maecenas suscipit, purus dictum congue condimentum, purus ante mollis leo, non tristique orci urna pretium nisl. Vestibulum nec nisi diam. Etiam viverra facilisis est non dignissim. Maecenas sodales elit quam, vel interdum mi placerat non. Quisque at metus faucibus mauris lobortis ultricies a sit amet quam. Mauris pretium neque sit amet sem porta, ornare dapibus mauris varius. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer facilisis nunc et pretium iaculis. Integer commodo risus id justo accumsan commodo. Sed sapien lacus, faucibus a elit a, commodo aliquet lectus.'
  },
  {
    id: 33,
    name: 'Game Title 10',
    price: 1299,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed viverra sollicitudin pellentesque. Maecenas suscipit, purus dictum congue condimentum, purus ante mollis leo, non tristique orci urna pretium nisl. Vestibulum nec nisi diam. Etiam viverra facilisis est non dignissim. Maecenas sodales elit quam, vel interdum mi placerat non. Quisque at metus faucibus mauris lobortis ultricies a sit amet quam. Mauris pretium neque sit amet sem porta, ornare dapibus mauris varius. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer facilisis nunc et pretium iaculis. Integer commodo risus id justo accumsan commodo. Sed sapien lacus, faucibus a elit a, commodo aliquet lectus.'
  },
  {
    id: 34,
    name: 'Game Title 11',
    price: 299,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed viverra sollicitudin pellentesque. Maecenas suscipit, purus dictum congue condimentum, purus ante mollis leo, non tristique orci urna pretium nisl. Vestibulum nec nisi diam. Etiam viverra facilisis est non dignissim. Maecenas sodales elit quam, vel interdum mi placerat non. Quisque at metus faucibus mauris lobortis ultricies a sit amet quam. Mauris pretium neque sit amet sem porta, ornare dapibus mauris varius. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer facilisis nunc et pretium iaculis. Integer commodo risus id justo accumsan commodo. Sed sapien lacus, faucibus a elit a, commodo aliquet lectus.'
  },
  {
    id: 35,
    name: 'Game Title 12',
    price: 199,
    currencyCode: 'UAH',
    tag: 'Indie',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed viverra sollicitudin pellentesque. Maecenas suscipit, purus dictum congue condimentum, purus ante mollis leo, non tristique orci urna pretium nisl. Vestibulum nec nisi diam. Etiam viverra facilisis est non dignissim. Maecenas sodales elit quam, vel interdum mi placerat non. Quisque at metus faucibus mauris lobortis ultricies a sit amet quam. Mauris pretium neque sit amet sem porta, ornare dapibus mauris varius. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer facilisis nunc et pretium iaculis. Integer commodo risus id justo accumsan commodo. Sed sapien lacus, faucibus a elit a, commodo aliquet lectus.'
  },
];
